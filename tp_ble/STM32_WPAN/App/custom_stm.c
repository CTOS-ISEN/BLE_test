...
typedef struct {
	...
	uint16_t CustomLcHdle; /**< led_char handle */
	uint16_t CustomScHdle; /**< switch_char handle */
	uint16_t CustomEsHdle; /**< echo_service handle */
	uint16_t CustomEccHdle; /**< echo_char handle */
...
#define COPY_LED_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xab,0xcd,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_SWITCH_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x12,0x34,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
#define COPY_ECHO_SERVICE_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0xbe,0xef,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_ECHO_CHAR_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0xcd,0xef,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
static SVCCTL_EvtAckStatus_t Custom_STM_Event_Handler(void *Event) {
	...
	case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
		...
		case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
			...
			if (attribute_modified->Attr_Handle
				...
				case (!(COMSVC_Notification)):
					...
					Notification.Custom_Evt_Opcode =
							CUSTOM_STM_SC_NOTIFY_DISABLED_EVT;
				...
				case COMSVC_Notification:
					...
					Notification.Custom_Evt_Opcode =
							CUSTOM_STM_SC_NOTIFY_ENABLED_EVT;
					Custom_STM_App_Notification(&Notification);
			...
			} /* if (attribute_modified->Attr_Handle == (CustomContext.CustomScHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/

			else if (attribute_modified->Attr_Handle
					== (CustomContext.CustomEccHdle
							+ CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET)) {
				return_value = SVCCTL_EvtAckFlowEnable;
				...
				case (!(COMSVC_Notification)):
					...
					Notification.Custom_Evt_Opcode =
							CUSTOM_STM_ECC_NOTIFY_DISABLED_EVT;
					Custom_STM_App_Notification(&Notification);
				...
				case COMSVC_Notification:
					...
					Notification.Custom_Evt_Opcode =
							CUSTOM_STM_ECC_NOTIFY_ENABLED_EVT;
					Custom_STM_App_Notification(&Notification);
			...
			} /* if (attribute_modified->Attr_Handle == (CustomContext.CustomEccHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))*/

			else if (attribute_modified->Attr_Handle
					== (CustomContext.CustomLcHdle
							+ CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET)) {
				return_value = SVCCTL_EvtAckFlowEnable;
				/* USER CODE BEGIN CUSTOM_STM_Service_1_Char_1_ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE */
				// +BCH
				Notification.Custom_Evt_Opcode =
						CUSTOM_STM_LC_WRITE_NO_RESP_EVT;
			...
			} /* if (attribute_modified->Attr_Handle == (CustomContext.CustomLcHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))*/
...
void SVCCTL_InitCustomSvc(void) {
	...
	PRIMARY_SERVICE, max_attr_record, &(CustomContext.CustomFsHdle));
	...
	UUID_TYPE_128, &uuid, SizeLc,
	CHAR_PROP_READ | CHAR_PROP_WRITE_WITHOUT_RESP,
	ATTR_PERMISSION_NONE,
	GATT_NOTIFY_ATTRIBUTE_WRITE, 0x10,
	CHAR_VALUE_LEN_CONSTANT, &(CustomContext.CustomLcHdle));
	if (ret != BLE_STATUS_SUCCESS) {
		APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : LC, error code: 0x%x \n\r", ret)
		;
	} else {
		APP_DBG_MSG("  Success: aci_gatt_add_char command   : LC \n\r")
		;
	}
	...
	ret = aci_gatt_add_char(CustomContext.CustomFsHdle,
	UUID_TYPE_128, &uuid, SizeSc,
	CHAR_PROP_NOTIFY,
	ATTR_PERMISSION_NONE,
	GATT_NOTIFY_ATTRIBUTE_WRITE, 0x10,
	CHAR_VALUE_LEN_CONSTANT, &(CustomContext.CustomScHdle));
	if (ret != BLE_STATUS_SUCCESS) {
		APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : SC, error code: 0x%x \n\r", ret)
		;
	} else {
		APP_DBG_MSG("  Success: aci_gatt_add_char command   : SC \n\r")
		;
	}
	 ...
	 * service_max_attribute_record = 1 for echo_service +
	 *                                2 for echo_char +
	 *                                1 for echo_char configuration descriptor +
	 *                              = 4
	 *
	 * This value doesn't take into account number of descriptors manually added
	 * In case of descriptors added, please update the max_attr_record value accordingly in the next SVCCTL_InitService User Section
	...
	COPY_ECHO_SERVICE_UUID(uuid.Char_UUID_128);
	ret = aci_gatt_add_service(UUID_TYPE_128, (Service_UUID_t*) &uuid,
	PRIMARY_SERVICE, max_attr_record, &(CustomContext.CustomEsHdle));
	if (ret != BLE_STATUS_SUCCESS) {
		APP_DBG_MSG("  Fail   : aci_gatt_add_service command: es, error code: 0x%x \n\r", ret)
		;
	} else {
		APP_DBG_MSG("  Success: aci_gatt_add_service command: es \n\r")
		;
	}
	 ...
	 *  echo_char
	 */
	COPY_ECHO_CHAR_UUID(uuid.Char_UUID_128);
	ret = aci_gatt_add_char(CustomContext.CustomEsHdle,
	UUID_TYPE_128, &uuid, SizeEcc,
	CHAR_PROP_NOTIFY,
	ATTR_PERMISSION_NONE,
	GATT_NOTIFY_ATTRIBUTE_WRITE, 0x10,
	CHAR_VALUE_LEN_CONSTANT, &(CustomContext.CustomEccHdle));
	if (ret != BLE_STATUS_SUCCESS) {
		APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : ECC, error code: 0x%x \n\r", ret)
		;
	} else {
		APP_DBG_MSG("  Success: aci_gatt_add_char command   : ECC \n\r")
		;
	}
	...
	switch (CharOpcode) {
		...
		case CUSTOM_STM_LC:
			ret = aci_gatt_update_char_value(CustomContext.CustomFsHdle,
					CustomContext.CustomLcHdle, 0, /* charValOffset */
					SizeLc, /* charValueLen */
					(uint8_t*) pPayload);
			if (ret != BLE_STATUS_SUCCESS) {
				APP_DBG_MSG("  Fail   : aci_gatt_update_char_value LC command, result : 0x%x \n\r", ret)
				;
			} else {
				APP_DBG_MSG("  Success: aci_gatt_update_char_value LC command\n\r")
				;
			}
		...
		case CUSTOM_STM_SC:
			ret = aci_gatt_update_char_value(CustomContext.CustomFsHdle,
					CustomContext.CustomScHdle, 0, /* charValOffset */
					SizeSc, /* charValueLen */
					(uint8_t*) pPayload);
			if (ret != BLE_STATUS_SUCCESS) {
				APP_DBG_MSG("  Fail   : aci_gatt_update_char_value SC command, result : 0x%x \n\r", ret)
				;
			} else {
				APP_DBG_MSG("  Success: aci_gatt_update_char_value SC command\n\r")
				;
			}
		...
		case CUSTOM_STM_ECC:
			ret = aci_gatt_update_char_value(CustomContext.CustomEsHdle,
					CustomContext.CustomEccHdle, 0, /* charValOffset */
					SizeEcc, /* charValueLen */
					(uint8_t*) pPayload);
			if (ret != BLE_STATUS_SUCCESS) {
				APP_DBG_MSG("  Fail   : aci_gatt_update_char_value ECC command, result : 0x%x \n\r", ret)
				;
			} else {
				APP_DBG_MSG("  Success: aci_gatt_update_char_value ECC command\n\r")
				;
			}
